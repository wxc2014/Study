...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	char className[] = "WIN3DCLASS";
	char winName[] = "WIN3D Game Console";
	...
	if(!(hwnd = CreateWindow(className,		//class
							winName,	//title
							WS_POPUP | WS_VISIBLE,
							0, 0,					//initial x, y
...
{
	//this function tests if the ball has hit block or the paddle
	...
	if(ball_y > (SCREEN_HEIGHT/2) && ball_dy > 0)
	{
			...
			&& (y >= paddle_y && y <= paddle_y + PADDLE_HEIGHT))
		{
			...
			if(KEY_DOWN(VK_RIGHT))	ball_dx -= (rand() % 3);
			else if(KEY_DOWN(VK_LEFT))	ball_dx += rand()%3;
			else	ball_dx += (-1 + rand() % 3);
	...
	for(int row=0; row < NUM_BLOCK_ROWS; row++)
	{
		...
		for(int col=0; col < NUM_BLOCK_COLUMNS; col++)
		{
			...
			if(blocks[row][col] != 0)
			{
				...
				if((ball_cx > x1) && (ball_cy < x1 + BLOCK_WIDTH)
...
int Game_Main(void * parms)
{
	...
	else if(game_state == GAME_STATE_RUN)
	{
		...
		//next wathc out for ball velocity getting out of hand
		...
		//draw the info
		sprintf(buffer, "FREAKOUT	Score %d // Level %d", score, level);
